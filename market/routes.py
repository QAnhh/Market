...
def market_page():
    ...
    if request.method == "POST":
        ...
        p_item_object = Item.query.filter_by(id=purchase_item).first()
        if p_item_object:
            if current_user.can_purchase(item_obj=p_item_object):
                p_item_object.buy(user=current_user)
                flash(f"Congratulations! You purchased {p_item_object.name} for {p_item_object.price}$", category="success")
            else:
                flash(f"Sorry, you don't enough money to purchase {p_item_object.name}", category="danger")
        #Sell Item logic
...
def register_page():
    ...
    if form.validate_on_submit():
        ...
        flash(f"Account created successfully! You are logged in as {user_create.username}", category='success')
        return redirect(url_for('market_page'))
    if form.errors != {}: #if have errors
